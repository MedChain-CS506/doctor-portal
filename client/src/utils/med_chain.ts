 ...
 * medChain API
 */
export class medChain extends SoltsiceContract {
    public static get artifacts() { return require('../contracts/med_chain.json'); }

    public static get bytecodeHash() {
        // we need this before ctor, but artifacts are static and we cannot pass it to the base class, so need to generate
        let artifacts = medChain.artifacts;
    ...
    public static async new(deploymentParams: W3.TX.TxParams, ctorParams?: {}, w3?: W3, link?: SoltsiceContract[], privateKey?: string): Promise<medChain> {
        w3 = w3 || W3.default;
        if (!privateKey) {
            let contract = new medChain(deploymentParams, ctorParams, w3, link);
            await contract._instancePromise;
            return contract;
        } else {
            let data = medChain.newData(ctorParams, w3);
            let txHash = await w3.sendSignedTransaction(W3.zeroAddress, privateKey, data, deploymentParams);
            let txReceipt = await w3.waitTransactionReceipt(txHash);
            let rawAddress = txReceipt.contractAddress;
            let contract = await medChain.at(rawAddress, w3);
            return contract;
        }
    }

    public static async at(address: string | object, w3?: W3): Promise<medChain> {
        let contract = new medChain(address, undefined, w3, undefined);
        await contract._instancePromise;
        return contract;
    }

    public static async deployed(w3?: W3): Promise<medChain> {
        let contract = new medChain('', undefined, w3, undefined);
        await contract._instancePromise;
        return contract;
    }
    ...
    public static newData(ctorParams?: {}, w3?: W3): string {
        // tslint:disable-next-line:max-line-length
        let data = SoltsiceContract.newDataImpl(w3, medChain.artifacts, ctorParams ? [] : []);
        return data;
    }
